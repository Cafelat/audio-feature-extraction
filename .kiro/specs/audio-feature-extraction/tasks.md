...
#### 完了条件
- [x] HDF5ファイルへの書き込みが動作する
- [x] 4種類のデータ形式すべてで保存できる
- [x] 各形式のチャンネル数が正しい（2/2/3/1）
- [x] データ形状が (time, freq, channels) になっている
- [x] 全メタデータが保存される
- [x] 圧縮が動作する
- [x] 単体テストで80%以上のカバレッジ（88%達成）
...
- `AudioMixer` クラス実装（設計書 4.3）
  - `mix_with_snr()`: 指定したSN比でクリーン信号とノイズを混合
    ...
    - **クリッピング対策**: 振幅が1.0を超える場合の正規化（headroom=0.01）
    - **実際のSNR再計算**: 正規化後のSNRをメタデータに記録
  - `_adjust_noise_length()`: ノイズの長さをクリーン信号に合わせる
  - `_calculate_rms()`: RMS計算
  - `batch_mix()`: バッチ処理（ランダムなノイズ・SNR選択）
  - サンプリングレートチェック
  - モノラル化処理
  - **メタデータ拡張**: ノイズ除去評価用の情報を保持
    - `snr_db_requested`: 指定されたSN比
    - `snr_db_actual`: クリッピング対策後の実際のSN比
    - `normalization_factor`: 正規化係数（クリッピング回避用）
    - `clipping_occurred`: クリッピング発生フラグ
...
- [ ] メタデータに混合情報が保存される
- [ ] **クリッピング検出と正規化が動作する**
- [ ] **メタデータにクリッピング対策情報が含まれる**（snr_db_actual, normalization_factor, clipping_occurred）
- [ ] **ノイズ除去評価時に振幅検証が可能**（メタデータを使って同じ大きさの信号確認）
- [ ] 単体テストで80%以上のカバレッジ
- [ ] SNRの検証テスト（実測値が目標値±1dB以内）
- [ ] クリッピング発生時のSNR変化テスト（snr_db_actual の正確性確認）
